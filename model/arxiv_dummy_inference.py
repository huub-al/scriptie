"""
arxiv_dummy_inference.py
Huub Al 

This script loads a trained model and performs inference on the dummy hypergraph.
"""
import sys
import os
import torch
...
def inference(args):
    ...
    # Create SetGNN model
    setgnn_model = SetGNN(model_args)
    ...
    setgnn_model.load_state_dict(checkpoint['setgnn_state_dict'])
    ...
    print(f"Number of edges: {len(edge_probs)}")
    print(f"Average edge probability: {edge_probs.mean().item():.4f}")
    print(f"Min edge probability: {edge_probs.min().item():.4f}")
    print(f"Max edge probability: {edge_probs.max().item():.4f}")
